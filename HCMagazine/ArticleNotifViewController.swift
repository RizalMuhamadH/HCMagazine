//
//  ArticleNotifViewController.swift
//  HCMagazine
//
//  Page to display an unread article with CSS setup and like/dislike feature (Article Notif Scene) with auto insert hits to be counted in news analytics triggered by local notification
//
//  Created by ayobandung on 9/5/17.
...
class ArticleNotifViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UIScrollViewDelegate{
    ...
    let contentURL = "http://mobs.ayobandung.com/index.php/news_controller/getDetailNews"
    ...
    override func viewDidLoad() {
        ...
        navBar.topItem?.title = barTitle
        username = defaults.string(forKey: "usr")!
        
        // Looks for single or multiple taps.
        let tap: UITapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(ArticleNotifViewController.dismissKeyboard))
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if (getHTML == "" && newsList.count==0) {
            ...
            // update comment, hits, likes counter
            hitsLabel.text = "\(totalHits)"
            commentLabel.text = "\(totalComments) orang berkomentar"
            likesLabel.text = "\(totalLikes) orang menyukai"
            dispatchQueue.async {
            ...
            do {
                ...
                if(state=="success"){
                    if mode==4{
                        ...
                        let newsData = json["data"].arrayValue
                        self.content = newsData[0]["news_content"].stringValue
                        self.totalLikes = newsData[0]["counter"].intValue
                        self.totalHits = newsData[0]["hits"].intValue
                        self.totalComments = newsData[0]["comment"].intValue
                    }
                }else{
